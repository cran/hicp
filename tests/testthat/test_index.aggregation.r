...
A <- dt[coicop%in%c("01111","01112"),
        list("coicop"="0111",
             "price"=laspey(x=price, w0=weight),
             "weight"=sum(weight),
...
setnames(x=res.own, c("time","grp","laspey","w0","wt","is_aggregated"))
...
# check for errors:
expect_error(
  dt[, aggregate(x=price, wt=weight, index=laspey), by="time"]
)

expect_error(
  dt[, aggregate(x=price, wt=weight, index=list(mean)), by="time"]
)

expect_error(
  dt[, aggregate(x=price, wt=weight, index=list("mean"=function(x) mean(x))), by="time"]
)
# END
